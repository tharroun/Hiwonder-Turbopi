...
class BoardSDK:
    ...
    def __init__(
        ...
        logging.basicConfig(filename='BoardSDK.log', filemode='w', level=logging.INFO)
        ...
        """
        If reception is enabled, activate these items.
        """
        ...
        """
        Center the servos:
        After booting, the board returns the servo postion as zero until it receives a set-position command.
        Then it reports the positions correctly.
        """
        self.set_servo_position(0.5,[(1,1500),(2,1500)])
        self.set_servo_offset(1,0)
        self.set_servo_offset(2,0)
        time.sleep(0.5)
    ...
    def get_servo_position(self, servo_id: int):
        if self.enable_recv:
            if servo_id > 2 or servo_id < 0:
                self.logger.warning(f"Servo id must be 1 or 2")
                return None
            with self.servo_read_lock:
                self._send_data_to_port(Functions.FUNC_PWM_SERVO.value,[0x05,servo_id])
                time.sleep(0.25)
                servo_id, cmd, info = struct.unpack("<BBH", self.queue_servo[0])
                return info
        else:
            self.logger.warning(f"Reception not enabled to read the servo offset")
            return None

    def get_servo_offset(self, servo_id: int):
        if self.enable_recv:
            if servo_id > 2 or servo_id < 0:
                self.logger.warning(f"Servo id must be 1 or 2")
                return None
            with self.servo_read_lock:
                self._send_data_to_port(Functions.FUNC_PWM_SERVO.value,[0x09,servo_id])
                time.sleep(0.25)
                servo_id, cmd, info = struct.unpack("<BBb", self.queue_servo[0])
                return info
        else:
            self.logger.warning(f"Reception not enabled to read the servo offset")
            return None
    ...
    def set_servo_position(self, duration: float, positions: List[Tuple[int, int]]) -> None:
        """
        TEST
        """
        ...
        for i in positions:
            if len(i) != 2:
                raise ValueError("Each position must be a tuple of integers: (index, position).")
            if i[0] > 2 or i[0] < 0:
                raise ValueError("Servo id muste be 1 or 2")
            if not (500<i[1]<2500):
                raise ValueError("Servo position must be between 1000 and 2000")
            data.extend(struct.pack("<BH", i[0], i[1]))
        self._send_data_to_port(Functions.FUNC_PWM_SERVO.value, data)
    
    def set_servo_offset(self, servo_id: int, offset: int) -> None:
        """
        TEST
        """
        if servo_id > 2 or servo_id < 0:
                raise ValueError("Servo id muste be 1 or 2")
        if offset > 127 or offset < -127:
                raise ValueError("Servo offset must be between -127 and 127")
    ...
    def set_rgb(self, pixels: List[Tuple[int, int, int, int]]) -> None:
        ...
        for pixel in pixels:
            ...
            if not (0 <= r <= 255 and 0 <= g <= 255 and 0 <= b <= 255):
                raise ValueError(f"RGB values must be in the range 0-255. Received: {r}, {g}, {b}")
...
if __name__ == "__main__":
    ...
    board.set_servo_position(0.1,[(1,1800),(2,1200)])
    time.sleep(0.1)
    
    p1 = board.get_servo_position(1)
    p2 = board.get_servo_position(2)
    if p1 != None : print(f"Servo 1 position: {p1}")
    if p2 != None : print(f"Servo 2 position: {p2}")

    p1 = board.get_servo_offset(1)
    p2 = board.get_servo_offset(2)
    if p1 != None : print(f"Servo 1 offset: {p1}")
    if p2 != None : print(f"Servo 2 offset: {p2}")

    board.set_motor_duty([(1,15.0),(2,15.0),(3,15.0),(4,15.0)])
    time.sleep(1.0)
